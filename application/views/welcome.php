...
<div class="middle-banner_bg"><!-- staing middlebanner -->
    ...
    <div class="home_text">
        ...
        <?php else: ?>
            <div id="home_info"><label><h2 style="font-size: 25px;">绿奇异让您，所见即所知！</h2></label>
        ...
        <div class="container Mcenter clearfix transitions-enabled masonry" id="alpha" style="height: 6247px; width: 1392px;">
            ...
            <?php if (is_array($boardPin)): ?>
                <?php foreach ($boardPin as $boardPinKey => $boardPinValue): ?>
                    ...
                    <?php $comments = getPinCommentsWithNum($boardPinValue->id,4); ?>
                    <?php $commentCount = getPinComments($boardPinValue->id); ?>
                    <div class="pin_item">
                        <?php $this->load->view('popup_js'); ?>
                        <?php if (!$this->session->userdata('login_user_id')): ?>
                            <div class="action">
                                <span id="like_action">
                                    <a class="act_like ajax" href="<?php echo site_url(); ?>board/pins/<?php echo $boardDetails->id; ?>/<?php echo $boardPinValue->id; ?>/view"><span>喜欢</span></a>
                                </span>
                                <a class="fancyboxForm act_repin ajax" href="<?php echo site_url(); ?>board/pins/<?php echo $boardDetails->id; ?>/<?php echo $boardPinValue->id; ?>/view">收藏</a>
                                <a class="act_comment ajax" href="<?php echo site_url(); ?>board/pins/<?php echo $boardDetails->id; ?>/<?php echo $boardPinValue->id; ?>/view" ><span>评论</span></a>
                            </div>
                        <?php else: ?>
                            <div class="action">
                                ...
                                <span id="like_action<?php echo $boardPinValue->id; ?>">
                                    <?php if ($boardPinValue->user_id == $this->session->userdata('login_user_id')): ?>
                                        <a href="<?php echo site_url('board/pinEdit/' . $boardPinValue->board_id . '/' . $boardPinValue->id) ?>" class="act_repin"><span>编辑</span></a>
                                    <?php else: ?>

                                        <?php if (!isLiked($boardPinValue->id, $this->session->userdata('login_user_id'))): ?>
                                            <a class="act_like" id="<?php echo $likeId ?>" href="javascript:;"  onClick="doAction(<?php echo $like; ?>,<?php echo $boardPinValue->id; ?>,'like')"><span>喜欢</span></a>
                                        <?php else: ?>
                                            <a class="act_unlike" id="<?php echo $unlikeId ?>" href="javascript:;"   onClick="doAction(<?php echo $like; ?>,<?php echo $boardPinValue->id; ?>,'unlike')"><span>已喜欢</span></a>
                                        <?php endif ?>
                                    <?php endif ?>
                                </span>
                                ...
                                <a class="ajax" href="<?php echo site_url('repin/load/' . $boardPinValue->id) ?>" >收藏</a>


                                <?php $commentId = 'comment-' . $boardPinValue->id ?>
                                <?php $uncommentId = 'uncomment-' . $boardPinValue->id ?>
                                <a class="act_comment" id="<?php echo $commentId ?>" href="javascript:;" onClick="addComment(<?php echo $boardPinValue->id; ?>,'comment')"><span>评论</span></a>
                                <a class="act_uncomment" id="<?php echo $uncommentId ?>" href="javascript:;" onClick="addComment(<?php echo $boardPinValue->id; ?>,'uncomment')" ><span>取消</span></a>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="comm_des">
                            ...
                            <p class="comm_like">
                                ...
                                <span id="<?php echo $likeCountId; ?>"><?php echo getPinLikeCount($boardPinValue->id); ?> 喜欢</span>
                                <span id="<?php echo $commentCountId; ?>"> <?php echo count($commentCount) ?> 评论</span>
                        ...
                        <div class="convo_blk attribution">
                            ...
                            <p>
                                ...
                                <?php if ($boardPinValue->source_url != ""): ?>
                                    通过 <a href="<?php echo $boardPinValue->source_url; ?>"><?php echo $source; ?></a>
                                <?php endif; ?>
